...
### ⚠️ Windows Editing Rules (MUST follow every time)

Any time `project.pbxproj` is edited on Windows (by any tool, including AI agents):

```powershell
# After ALL edits — strip BOM and verify header
$path = "ios/Runner.xcodeproj/project.pbxproj"
$bytes = [System.IO.File]::ReadAllBytes($path)
if ($bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
    $bytes = $bytes[3..($bytes.Length-1)]
...
**Never use `StrReplace`, `Write`, or `[System.Text.Encoding]::UTF8` to save this file** — they all add BOM. Always use `[System.IO.File]::WriteAllBytes` for final write.

---

### 1. PBXBuildFile section — NSE entries
...
### 2. PBXContainerItemProxy — NSE proxy
...
⚠️ These go in `/* Begin PBXFileReference section */` ONLY. Never inside a `children = (...)` array.

---

### 5. PBXGroup — Runner group children
...
⚠️ `children` arrays contain ONLY `UUID /* name */,` references — NEVER full `= {isa = ...}` definitions.
...
97C146ED1CF9000F007C117D /* Runner */ = {
    ...
    buildRules = ();
    dependencies = (
        NSE_TARGET_DEP /* PBXTargetDependency */,   ← ONE block only
    );
    name = Runner;
    productName = Runner;
    productReference = 97C146EE1CF9000F007C117D /* Runner.app */;
...
⚠️ Runner MUST have exactly **one** `dependencies` key. Two `dependencies` blocks → `Multiple commands produce .appex`.
...
### 8. PBXProject — TargetAttributes (NSE entry goes LAST)
...
TargetAttributes = {
    ...
    NSE_TARGET = {
        CreatedOnToolsVersion = 15.0;   ← NSE goes last, version 15.0
    };
};
```
...
### 9. XCBuildConfiguration — NSE (all 3: Debug, Release, Profile)
...
NSE_DEBUG_CFG /* Debug */ = {
    ...
    buildSettings = {
        ...
        PRODUCT_NAME = "$(TARGET_NAME)";      ← MUST NOT be empty string ""
        SDKROOT = iphoneos;
        SKIP_INSTALL = YES;                   ← MUST be YES
...
|---------|--------------|-------------|-----|
| `PRODUCT_NAME` | `"$(TARGET_NAME)"` | `""` | Empty → Xcode can't resolve .appex name → `Multiple commands produce .appex` |
| `CURRENT_PROJECT_VERSION` | `1` (hardcoded) | `$(FLUTTER_BUILD_NUMBER)` | xcconfig variable → CocoaPods can't set base xcconfig → Firebase not linked |
...
### 10. NSE Resources phase — EMPTY

```
NSE_RESOURCES_PHASE /* Resources */ = {
    isa = PBXResourcesBuildPhase;
    buildActionMask = 2147483647;
    files = ();   ← EMPTY — do NOT add Info.plist here
    runOnlyForDeploymentPostprocessing = 0;
};
```

⚠️ Adding `Info.plist` to Resources causes `Multiple commands produce ... Info.plist` because `INFOPLIST_FILE` build setting already handles it.

---

### 11. XCConfigurationList — NSE
