...
| `Multiple commands produce '.../.appex'` (пустое имя файла — `.appex` без `NotificationService`) | PowerShell раскрыл `"$(TARGET_NAME)"` как пустую строку в NSE build configs (`PRODUCT_NAME = ""`) | Заменить `PRODUCT_NAME = "";` на `PRODUCT_NAME = "$(TARGET_NAME)";`. **Профилактика:** при добавлении NSE через PowerShell here-string — экранировать $ через backtick: `` PRODUCT_NAME = "`$(TARGET_NAME)" `` |
