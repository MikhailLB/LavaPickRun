...
class TrackingSignal {
  ...
  Future<void> _doWarmup() async {
    ...
    debugPrint('[LPR.TS] warmup devKeyLen=${devKey.length}');
    if (devKey.isEmpty) {
      _started = true;
    ...
    try {
      ...
      debugPrint('[LPR.TS] initSdk OK');
    } catch (err, st) {
      debugPrint('[LPR.TS] warmup error: $err\n$st');
      if (!_conversionDone.isCompleted) _conversionDone.complete({});
      if (!_deepLinkDone.isCompleted) _deepLinkDone.complete();
    }
  }
  ...
  Future<void> _requestAtt() async {
    try {
      ...
      debugPrint('[LPR.TS] ATT skipped: $err');
    }
  }
  ...
  void _onConversion(dynamic raw) async {
    ...
    debugPrint('[LPR.TS] conversion ${jsonEncode(data)}');
  ...
  Future<Map<String, dynamic>> buildPayload({
    ...
    debugPrint('[LPR.TS] payload keys=${body.keys.toList()}');
    return body;
  }
}
