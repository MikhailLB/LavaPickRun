...
class GateDispatch {
  ...
  Future<GateReply> send(Map<String, dynamic> body) async {
    ...
    debugPrint('[LPR.GD] send → endpoint="$endpoint"');
    if (endpoint.isEmpty) {
      debugPrint('[LPR.GD] endpoint not configured — declined');
      return GateReply.declined('endpoint_missing');
    }
    try {
      final uri = Uri.parse(endpoint);
      debugPrint('[LPR.GD] POST $uri  body=${jsonEncode(body)}');
      ...
      debugPrint('[LPR.GD] HTTP ${resp.statusCode}');
      final preview = resp.body.length > 500
          ? '${resp.body.substring(0, 500)}…'
          : resp.body;
      debugPrint('[LPR.GD] body=$preview');
      ...
      debugPrint('[LPR.GD] reply granted=${reply.granted} dest=${reply.destination}');
